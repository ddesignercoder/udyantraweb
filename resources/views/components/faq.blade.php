{{-- Section 6: FAQ --}}
    ...
    <div class="max-w-3xl mx-auto px-4">
        <h2 class="text-sizeMobile lg:text-sizeDesktop font-semibold text-center text-black font-sans mb-4 md:mb-10">
        ...
        <div class="space-y-4" x-data="{ active: null }">
            @foreach($faqs as $faq)
                <div class="transition-all duration-300">
                    <button @click="active = (active === {{ $loop->index }} ? null : {{ $loop->index }})" class="w-full px-8 py-4 md:py-5 flex items-start justify-between text-left focus:outline-none cursor-pointer bg-white rounded-4xl">
                    ...
                    <div x-show="active === {{ $loop->index }}" x-collapse x-cloak class="px-8 md:pb-6 text-black text-base fw-normal  md:mt-5 leading-relaxed">
                        {{ $faq['answer'] }}
                    </div>
                </div>
            @endforeach
